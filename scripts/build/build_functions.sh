...
BuildLArContent() {
    ...
    RunWithError cd $PANDORA_PROJECT_DIR
    RunWithError git clone https://github.com/GianfrancoIngratta/LArContent.git
    RunWithError cd LArContent
    # RunWithError git checkout $PANDORA_LAR_CONTENT_VERSION
    RunWithError git checkout feature/2x2_workshop_sept2026_w_printPfoInfo_x_validation
    RunWithError mkdir build
    RunWithError cd build
